	...
	virtual tsl::elm::Element* createUI() override {
		...
		list->addItem(new tsl::elm::CustomDrawer([](tsl::gfx::Renderer *renderer, s32 x, s32 y, s32 w, s32 h) {
			if (!SaltySD) {
				...
				if (!*pluginActive) renderer->drawString("游戏未检查当前模式！", false, x, y+40, 18, renderer->a(0xF33F));
				else {
	...
	virtual void update() override {
		...
		if (PluginRunning) {
			if (i > 9) {
				...
				if (!res_mode_ptr->handheld_res) strcpy(HandheldDDR, "手持DDR：默认");
				else snprintf(HandheldDDR, sizeof(HandheldDDR), "手持DDR：%dx%d", resolutions[res_mode_ptr->handheld_res].first, resolutions[res_mode_ptr->handheld_res].second);
				if (!res_mode_ptr->docked_res) strcpy(DockedDDR, "底座DDR：默认");
				else snprintf(DockedDDR, sizeof(DockedDDR), "底座DDR：%dx%d", resolutions[res_mode_ptr->docked_res].first, resolutions[res_mode_ptr->docked_res].second);
			}
			else i++;
		}
