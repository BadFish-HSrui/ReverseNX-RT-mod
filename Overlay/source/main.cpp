...
public:
	...
	virtual tsl::elm::Element* createUI() override {
		...
		auto frame = new tsl::elm::OverlayFrame("ReverseNX-RT", _isDocked ? "更改底座模式默认分辨率" : "更改手持模式默认分辨率");
...
public:
	...
	virtual tsl::elm::Element* createUI() override {
		// A OverlayFrame is the base element every overlay consists of. This will draw the default Title and Subtitle.
		// If you need more information in the header or want to change it's look, use a HeaderOverlayFrame.
		auto frame = new tsl::elm::OverlayFrame("ReverseNX-RT", "葡萄糖酸菜鱼 汉化");
		...
		list->addItem(new tsl::elm::CustomDrawer([](tsl::gfx::Renderer *renderer, s32 x, s32 y, s32 w, s32 h) {
			if (!SaltySD) {
				...
				if (closed) {
					renderer->drawString("游戏已关闭！插件已禁用！", false, x, y+20, 19, renderer->a(0xF33F));
				}
				else {
				...
				renderer->drawString("ReverseNX-RT 已运行。", false, x, y+20, 20, renderer->a(0xFFFF));
				if (!(ReverseNX_RT->pluginActive)) renderer->drawString("游戏未检查当前模式！", false, x, y+40, 18, renderer->a(0xF33F));
				else {
		...
		if (PluginRunning && ReverseNX_RT->pluginActive) {
			...
			if (!(ReverseNX_RT->def)) {
				...
				if (ReverseNX_RT->wasDDRused) {
					auto *clickableListItem3 = new tsl::elm::ListItem("更改手持DDR");
					...
					auto *clickableListItem4 = new tsl::elm::ListItem("更改底座DDR");
			...
			auto *clickableListItem3 = new tsl::elm::ListItem("保存设置");
			clickableListItem3->setClickListener([](u64 keys) { 
				if ((keys & HidNpadButton_A) && PluginRunning) {
					if (writeSave())
						snprintf(saveChar, sizeof(saveChar), "保存成功！");
					else snprintf(saveChar, sizeof(saveChar), "保存失败！");
					return true;
				}
	...
	virtual void update() override {
		...
		if (PluginRunning) {
			if (i > 9) {
				...
				if (_def) {
					if (_isDocked) sprintf(DockedChar, "系统：底座");
					else sprintf(DockedChar, "系统：手持");
				}
				else {
					if (_isDocked) sprintf(DockedChar, "自定义：底座");
					else sprintf(DockedChar, "自定义：手持");
				}

				if (!ReverseNX_RT->res.handheld_res) strcpy(HandheldDDR, "手持DDR：默认");
				else snprintf(HandheldDDR, sizeof(HandheldDDR), "手持DDR：%dx%d", resolutions[ReverseNX_RT->res.handheld_res].first, resolutions[ReverseNX_RT->res.handheld_res].second);
				if (!ReverseNX_RT->res.docked_res) strcpy(DockedDDR, "底座DDR：Default");
				else snprintf(DockedDDR, sizeof(DockedDDR), "底座DDR：%dx%d", resolutions[ReverseNX_RT->res.docked_res].first, resolutions[ReverseNX_RT->res.docked_res].second);
			}
			else i++;
		}
